...
<div class="row">
  <div class="col-sm-6">
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <?php
          echo select_from('LevelDesc',"ac_m_LevelDescsAchStds WHERE Level = 'Year 10'");
        ?>
      </div>
    </div>
  </div>
  <div class="col-sm-6">
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <?php
          echo select_from('AchStd',"ac_m_LevelDescsAchStds WHERE Level = 'Year 10'");
        ?>
      </div>
    </div>
  </div>
...
foreach($s0 as $row0)
{
  ...
  foreach($s1 as $row1)
  {
    ...
    $q2 = "SELECT CdId,CdCode,ContentDesc,hLevel
    ...
    $s2->bindValue(':b',Config::read('app.hlevel.year10'),PDO::PARAM_STR);
    $s2->bindValue(':c',Config::read('app.hlevel.year10a'),PDO::PARAM_STR);
    $s2->execute();
    ...
    foreach($s2 as $row2)
    {
      ...
      echo "<dl>";
      if($row2['hlevel'] == Config::read('app.hlevel.year10a'))
      {
        echo "<dt>{$row2['cdcode']} <span class=\"label label-default\">Year 10A</span></dt>";
      }
      else
      {
